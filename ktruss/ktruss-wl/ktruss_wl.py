...
ast = Module([
    ...
    Kernel("find_mirror_edge", [graph.param(), 
                                ('index_type', 'u'), 
                                ('index_type', 'v'),],
           [CDecl([('index_type', 'start', ''),
                   ('index_type', 'end', ''),
                   ('index_type', 'mid', ''),
                   ('index_type', 'dst', '')]),
            
            CBlock("start = graph.getFirstEdge(v)"),
            CBlock("end = graph.getFirstEdge(v+1)"),

            While('start < end', 
                  [CBlock(["mid = (start + end) / 2",
                           "dst = graph.getAbsDestination(mid)"]),
                   If("dst == u", [CBlock("return mid")]),
                   If("u < dst", [CBlock("end = mid")], [CBlock("start = mid + 1")]),                           
               ]),
            CBlock("assert(false)")
