...
ast = Module([
  ...
  Kernel("selectivity", [dgraph.param(), qgraph.param(), ('int*', 'dprop_ptr'), ('int*', 'qprop_ptr')],
      [
        ForAll("node", qgraph.nodes(), [
          ...
          CBlock(['printf("node: %d, degree: %d, freq: %d, selectivity: %f\\n", node, degree, freq, selectivity)']),
          ])
      ]
      ),
  Kernel("gg_main", [params.GraphParam('hg', True), params.GraphParam('gg', True), params.GraphParam('qhg', True), params.GraphParam('qgg', True), ('Shared<int>&', 'dprop'), ('Shared<int>&', 'qprop')],
      [
          Invoke('selectivity', ('gg', 'qgg', 'dprop.gpu_rd_ptr()', 'qprop.gpu_rd_ptr()')),
      ])
])
